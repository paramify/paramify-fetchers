...
Pulls ITSM change records from the ServiceNow Paramify ITSM API and writes
the raw JSON response as evidence. Supports an optional watermark
...
logger = logging.getLogger("servicenow_changes")
...
def main() -> int:
    ...
    if resp.status_code == 403:
        report_failure(
            "ServiceNow denied access to changes endpoint (HTTP 403)", "not_authorized"
        )
        return 1
    if resp.status_code != 200:
        report_failure(
            f"ServiceNow changes endpoint returned HTTP {resp.status_code}: {resp.text[:300]}",
            "partial_failure",
        )
        return 1

    output_path = output_dir / "servicenow_changes.json"
    try:
